...
struct ProgressbarView: View {
	...
	var body: some View {
		let repeatWidth = (audioPlayer.repeatEndPoint - audioPlayer.repeatStartPoint) * viewWidth
		return Group {
			if audioPlayer.isRepeatPlaying{
				base.overlay(
					Rectangle()
						.opacity(0.3)
						.foregroundColor(.red)
						.frame(width: repeatWidth)
						.offset(x: repeatWidth/2+(audioPlayer.repeatStartPoint-0.5)*viewWidth)
				)
			}else{
				base
			}
